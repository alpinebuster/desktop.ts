...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerBuiltInAgents(instantiationService: IInstantiationService, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): IDisposable {
		return instantiationService.invokeFunction(accessor => {
			...
			const { disposable: vscodeDisposable } = SetupAgent.doRegisterAgent(instantiationService, chatAgentService, 'setup.vscode', 'vscode', false, localize2('vscodeAgentDescription', "Ask questions about VSCodium").value, ChatAgentLocation.Chat, ChatModeKind.Agent, context, controller);
			...
			disposables.add(SetupTool.registerTool(instantiationService, {
				...
				modelDescription: 'Scaffold a new workspace in VSCodium',
				userDescription: localize('setupToolsDescription', "Scaffold a new workspace in VSCodium"),
