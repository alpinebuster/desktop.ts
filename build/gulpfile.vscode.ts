...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(jsonEditor({ commit, date: readISODate(out), checksums, version, serverDownloadUrlTemplate: 'https://github.com/VSCodium/vscodium/releases/download/1.112.01962/vscodium-reh-${os}-${arch}-1.112.01962.tar.gz' }))
			.pipe(es.through(function (file) {
