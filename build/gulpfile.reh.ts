...
function packageTask(type: string, platform: string, arch: string, sourceFolderName: string, destinationFolderName: string) {
	...
	return () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(jsonEditor({ commit, date: readISODate(sourceFolderName), version, serverDownloadUrlTemplate: 'https://github.com/VSCodium/vscodium-insiders/releases/download/1.112.02078-insider/vscodium-reh-${os}-${arch}-1.112.02078-insider.tar.gz' }))
			.pipe(es.through(function (file) {
